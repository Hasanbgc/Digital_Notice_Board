...
inline fun <T, E : ResultError> Results<T, E>.onSuccess(action: (T) -> Unit): Results<T, E> {
    return when (this) {
        ...
        is Results.Success -> this.also { action(data) }
    }
}

inline fun <T, E : ResultError> Results<T, E>.onError(action: (E) -> Unit): Results<T, E> {
    return when (this) {
        is Results.Error -> this.also { action(error) }
...
sealed interface ResultError {
    ...
    sealed class Remote(override val message: String) : ResultError {
        ...
        object BAD_REQUEST : Remote("Invalid request. Please check your input.")
        object FORBIDDEN : Remote("You don't have permission to do that.")
        object NOT_FOUND : Remote("The requested resource was not found.")
    }
